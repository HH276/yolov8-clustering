...
parser.add_argument('--lr_critic', type=float, default=0.00001)#判别器的能力都要比生成器的能力要强,试试把判别器的学习率调小
parser.add_argument('--beta1', type=float, default=0.9)
parser.add_argument('--beta2', type=float, default=0.999)

# training configs
parser.add_argument('--training_type', type=str, default='Train')
parser.add_argument('--results_path', type=str, default='./logs/20231203_fenkai_lossD*10')
# parser.add_argument('--results_path', type=str, default='./logs_20231120_EMA_MADDG_DWR')
...
parser.add_argument('--W_trip', type=int, default=0.5)
parser.add_argument('--W_gen', type=int, default=1.0)
parser.add_argument('--W_intra', type=int, default=0.1)
parser.add_argument('--W_genave', type=int, default=1 / 3)
parser.add_argument('--W_yolo', type=int, default=1.0)  # yolo 的bbox的损失值 的权重系数
...
if __name__ == "__main__":
    ...
    #       在终端中输入    CUDA_VISIBLE_DEVICES=3 python train_beifen.py
    #   DDP模式：
    #       设置            distributed = True
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = '/disk/home/wurx/yolov8-pytorch-master/model_data/yolov8_s.pth'
    # model_path = '/disk/home/wurx/yolov8-pytorch-master/logs_20231120_EMA_MADDG_DWR/last_epoch_weights.pth'
    # model_path_backbone = '/disk/home/wurx/yolov8-pytorch-master/model_data/yolov8_s.pth'
    # ------------------------------------------------------#
    ...
    # save_dir = 'logs_20231120_EMA_MADDG_DWR'
    # save_dir = 'test'
    # ------------------------------------------------------------------#
    ...
    # ------------------------------------------------------#
    # train_annotation_path = '/disk/home/wurx/yolov8_beifen/2007_train.txt'
    # val_annotation_path = '/disk/home/wurx/yolov8_beifen/2007_val.txt'
    train_annotation_path = './20231124_chongzu_data_train.txt'
    val_annotation_path   = './20231124_chongzu_data_val.txt'
    ...
    # FeatExtmodel = feature123

    FeatExtmodel_pre1 = models.create(arch_FeatExt)
    ...
    # FeatExtS1_restore = osp.join('/disk/home/wurx/MADDoG-master/results', 'Pre_train', 'snapshots', args.dataset1, 'DGFA-Ext-final.pt')  # /disk/home/wurx/MADDoG-master/results/Pre_train/snapshots
    # FeatExtS2_restore = osp.join('/disk/home/wurx/MADDoG-master/results', 'Pre_train', 'snapshots', args.dataset2, 'DGFA-Ext-final.pt')  # 这个路径下相应的，对应的预训练权重。
    # FeatExtS3_restore = osp.join('/disk/home/wurx/MADDoG-master/results', 'Pre_train', 'snapshots', args.dataset3, 'DGFA-Ext-final.pt')
    FeatExtS1_restore = osp.join('/disk/home/wurx/yolov8-pytorch-master/results', 'Pre_train', 'snapshots', args.dataset1, 'Japan.pth')  # /disk/home/wurx/MADDoG-master/results/Pre_train/snapshots
    FeatExtS2_restore = osp.join('/disk/home/wurx/yolov8-pytorch-master/results', 'Pre_train', 'snapshots', args.dataset2, 'India.pth')  # 这个路径下相应的，对应的预训练权重。
    ...
    PreFeatExtorS1 = init_model(net=FeatExtmodel_pre1, init_type=args.init_type, restore=FeatExtS1_restore).to(device)
    PreFeatExtorS2 = init_model(net=FeatExtmodel_pre2, init_type=args.init_type, restore=FeatExtS2_restore).to(device)
    PreFeatExtorS3 = init_model(net=FeatExtmodel_pre3, init_type=args.init_type, restore=FeatExtS3_restore).to(device)
    ...
    # FeatExtor = init_model(net=FeatExtmodel, init_type=args.init_type, restore=FeatExt_restore).to("cuda")
    ...
    backbone = Backbone(phi, pretrained=pretrained).to(device)
    head = YoloBody(backbone=backbone,input_shape=input_shape, num_classes=num_classes, phi=phi, pretrained=pretrained).to('cuda')
    if model_path != '':
        if local_rank == 0:
            print('Load weights {}.'.format(model_path))
        pretrained_dict = torch.load(model_path, map_location=device)

        model_dict_backbone = backbone.state_dict()
        # load_key_backbone, no_load_key_backbone, load_key_head, no_load_key_head, temp_dict_backbone,temp_dict_head = [], [], [], [], {}, {}
        load_key_backbone, no_load_key_backbone, temp_dict_backbone = [], [], {}
        load_key, no_load_key, temp_dict_head = [], [], {}
        model_dict_head = head.state_dict()


        for k, v in pretrained_dict.items():
            #因为backbone被单独分开，而原始权重会有backbone.   这样子无法使用，因此要把backbone.的字符串去除掉再加载。
            if 'backbone.' in k:
                # print("???",k)
                temp_dict_backbone[k[9:]] = v
                load_key_backbone.append(k)
            else:
                if k in model_dict_head.keys() and np.shape(model_dict_head[k]) == np.shape(v):
                    temp_dict_head[k] = v
                    load_key.append(k)
                else:
                    no_load_key.append(k)
        model_dict_backbone.update(temp_dict_backbone)
        backbone.load_state_dict(model_dict_backbone)
        model_dict_head.update(temp_dict_head)
        head.load_state_dict(model_dict_head)
        # ####加载head的预训练权重
        # model_dict_head = head.state_dict()
        # load_key, no_load_key, temp_dict_head = [], [], {}
        # for k, v in pretrained_dict.items():
        ...
        if local_rank == 0:
            ...
            print("\nSuccessful Load Key:", str(load_key_backbone)[:500], "……\nSuccessful Load Key Num:", len(load_key_backbone))
            print("\nFail To Load Key:", str(no_load_key_backbone)[:500], "……\nFail To Load Key num:", len(no_load_key_backbone))
    ...
    if local_rank == 0:
        ...
        loss_history = LossHistory(log_dir, head, input_shape=input_shape)
    else:
        loss_history = None
    ...
    if Cuda:
        if distributed:
            ...
            model_train = torch.nn.DataParallel(head)
    ...
    #   权值平滑
    # ----------------------------#
    ema_backbone = ModelEMA(backbone)
    ...
    if True:
        ...
        # }[optimizer_type]
        optimizer = {
            'adam': optim.Adam(head.parameters(), Init_lr_fit, betas=(momentum, 0.999)),
            'sgd': optim.SGD(head.parameters(), Init_lr_fit, momentum=momentum, nesterov=True)
        }[optimizer_type]  ###yolo的优化器，只更新head
        # optimizer.add_param_group({"params": pg1, "weight_decay": weight_decay})
        # optimizer.add_param_group({"params": pg2})
        # optimizer.add_param_group({"params": itertools.chain(backbone.parameters())})
        # optimizer.add_param_group({"params": itertools.chain(FeatEmbder.parameters())})
        ...
        gen_val = DataLoader(val_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                             pin_memory=True,
                             drop_last=True, collate_fn=yolo_dataset_collate_eval, sampler=val_sampler,
                             worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        ...
        # optimizer_DG_conf = optim.Adam(itertools.chain(model.backbone.parameters()),
        #                                lr=args.lr_DG_conf, betas=(args.beta1, args.beta2))
        optimizer_DG_conf = optim.Adam(itertools.chain(backbone.parameters(), FeatEmbder.parameters()),
                                       lr=args.lr_DG_conf, betas=(args.beta1, args.beta2))
        ...
        # ----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(backbone, head, input_shape, class_names, num_classes, val_lines, log_dir, Cuda, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # fit_one_epoch(model_train, model, ema, ema_backbone, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step,
            fit_one_epoch(model_train, head, backbone, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step,
                          epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,
                          Unfreeze_batch_size,
                          Discriminator1, Discriminator2, Discriminator3, PreFeatExtorS1, PreFeatExtorS2,
                          PreFeatExtorS3,
                          # backbone,FeatEmbder, Discriminator1, Discriminator2, Discriminator3, PreFeatExtorS1,PreFeatExtorS2, PreFeatExtorS3,
                          current_epoch, criterionAdv, args, TripletLossCal, optimizer_DG_conf,
                          optimizer_critic1, optimizer_critic2, optimizer_critic3, summary_writer, saver,
                          local_rank)
