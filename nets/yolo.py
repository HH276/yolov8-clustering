...
#
#         # self.conv3_for_upsample2_EMA = EMA(channels=base_channels * 4, factor=8)
#         # self.conv3_for_downsample1_EMA = EMA(channels=base_channels * 8, factor=8)
#         # self.conv3_for_downsample2_EMA = EMA(channels=int(base_channels * 16 * deep_mul), factor=8)
#         # ------------------------加强特征提取网络------------------------#
#
...
#         self.stride = torch.tensor([256 / x for x in [8, 16, 32]])
...
#         #wrq增加dyhead 检测头
#         # self.dyhead = Detect_DyHead(nc=4, hidc=256, block_num=2, ch=(128, 256, 512))
#
#     def fuse(self):
#         print('Fusing layers... ')
...
#         # P3 = self.conv3_for_upsample2_EMA(P3)
#
#         # 256, 80, 80 => 256, 40, 40
...
#         # P4 = self.conv3_for_downsample1_EMA(P4)
#
#         # 512, 40, 40 => 512, 20, 20
...
#         # P5 = self.conv3_for_downsample2_EMA(P5)
#         # ------------------------加强特征提取网络------------------------#
#         # P3 256, 80, 80
#         # P4 512, 40, 40
...
#         # _, dbox, cls, x, anchors, strides = self.dyhead(x)
#
#         # y if self.export else (y, x), dbox, cls, self.anchors.to(dbox.device), self.strides.to(dbox.device)
#         # return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device)
#         # return dbox, cls, x, anchors.to(dbox.device), strides.to(dbox.device),feat3
#         # return dbox, cls, x, anchors.to(dbox.device), strides.to(dbox.device)
#
...
#         dbox = self.dfl(box)
#
#         return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device),feat3
#         # return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device)
#
#
# class EMA(nn.Module):
...
#         # return y if self.export else (y, x), dbox, cls, x , self.anchors.to(dbox.device), self.strides.to(dbox.device)
#         return dbox, cls, x , self.anchors.to(dbox.device), self.strides.to(dbox.device)
#
#     def bias_init(self):
...
# from nets.yolo_training import weights_init
# from utils.utils_bbox import make_anchors
# from nets.backbone import SimAM
#
# def fuse_conv_and_bn(conv, bn):
#     # 混合Conv2d + BatchNorm2d 减少计算量
#     # Fuse Conv2d() and BatchNorm2d() layers https://tehnokv.com/posts/fusing-batchnorm-and-conv/
...
#         self.ema1 = EMA(channels=128, factor=8)#这里256是随便填的 ，后面使用就是适配相应的通道  256， 512， 1024 * x
...
#         # feat1 = self.ema1(feat1)
#         # feat2 = self.ema2(feat2)
#         # feat3= self.ema3(feat3)
#         # ------------------------加强特征提取网络------------------------#
#         # 1024 * deep_mul, 20, 20 => 1024 * deep_mul, 40, 40
#         P5_upsample = self.upsample(feat3)
#         # 1024 * deep_mul, 40, 40 cat 512, 40, 40 => 1024 * deep_mul + 512, 40, 40
...
#
#
# # #上面是wrq改过的yolobody，下面是源码的yolobody，用于后面的验证用。
# # import numpy as np
# # import torch
# # import torch.nn as nn
# #
...
class YoloBody(nn.Module):
    ...
    def __init__(self,input_shape, num_classes, phi, pretrained=False):
        ...
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        # self.reg_max = ch[0] // 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = num_classes + self.reg_max * 4  # number of outputs per anchor
        self.num_classes = num_classes
    ...
    def forward(self, x):
        ...
        return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device), feat3, feat2, feat1#feat3  20,  feat2  40, feat1  80
