...
# #         # dbox, cls, origin_cls, anchors, strides, _ = inputs
...
class DecodeBox():
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5,
        ...
        output = [None for _ in range(len(prediction))]
        from nets.losses import relu_evidence
        for i, image_pred in enumerate(prediction):
            # ----------------------------------------------------------#
            #   对种类预测部分取max。
            #   class_conf  [num_anchors, 1]    种类置信度
            ...
            #不确定性分析
            _, class_pred = torch.max(image_pred[:, 4:4 + num_classes], 1, keepdim=True)
            evidence = relu_evidence(image_pred[:, 4:4 + num_classes])
            alpha = evidence + 1
            uncertainty = num_classes / torch.sum(alpha, dim=1, keepdim=True)
            class_conf = 1 - uncertainty
            # print('# ----------------------------------------------------------#')
